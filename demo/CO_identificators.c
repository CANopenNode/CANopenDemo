 ...
 */
static ODR_t OD_read_1008(OD_stream_t *stream, void *buf,
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_1009(OD_stream_t *stream, void *buf,
                          OD_size_t count, OD_size_t *countRead)
{
    if (stream == NULL || buf == NULL || countRead == NULL) {
        return ODR_DEV_INCOMPAT;
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_read_100A(OD_stream_t *stream, void *buf,
                          OD_size_t count, OD_size_t *countRead)
{
    if (stream == NULL || buf == NULL || countRead == NULL) {
        return ODR_DEV_INCOMPAT;
...
OD_extension_t OD_1008_extension = {
    .object = NULL,
    .read = OD_read_1008,
    .write = NULL
};
OD_extension_t OD_1009_extension = {
    .object = NULL,
    .read = OD_read_1009,
    .write = NULL
};
OD_extension_t OD_100A_extension = {
    .object = NULL,
    .read = OD_read_100A,
    .write = NULL
};
...
void CO_identificators_init(uint16_t *bitRate, uint8_t *nodeId) {
    ...
    if (*bitRate == 0) *bitRate = CO_BITRATE_INITIAL;
    if (*nodeId == 0) *nodeId = CO_NODE_ID_INITIAL;
    ...
    OD_extension_init(OD_ENTRY_H1008_manufacturerDeviceName,
                      &OD_1008_extension);
    OD_extension_init(OD_ENTRY_H1009_manufacturerHardwareVersion,
                      &OD_1009_extension);
    OD_extension_init(OD_ENTRY_H100A_manufacturerSoftwareVersion,
                      &OD_100A_extension);
