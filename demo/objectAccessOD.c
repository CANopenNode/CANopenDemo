 ...
 */
static ODR_t OD_read_demoRecord(OD_stream_t *stream, void *buf,
 ...
 * For more information see file CO_ODinterface.h, OD_IO_t.
 */
static ODR_t OD_write_demoRecord(OD_stream_t *stream, const void *buf,
                                     ...
                                     uint32_t *errInfo)
{
    ...
    if (thisObj->i64 == NULL || thisObj->u64 == NULL
        || thisObj->r32 == NULL || thisObj->r64 == NULL
    ) {
        *errInfo = OD_getIndex(OD_demoRecord);
        return CO_ERROR_OD_PARAMETERS;
    }
    ...
    odRet = OD_getSub(OD_demoRecord, SUBIDX_AVERAGE,
                      &thisObj->io_average, false);
    if (odRet != ODR_OK) {
        *errInfo = OD_getIndex(OD_demoRecord);
        return CO_ERROR_OD_PARAMETERS;
    }
    ...
    odRet = OD_get_u16(OD_demoRecord, SUBIDX_PARAMETER,
                       &parameterU16, true);
